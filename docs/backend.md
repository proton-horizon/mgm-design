...
Generated JPEGs live under `mgm-previews/<board>/<frame>.jpg` in the uploaded bundle; source files are untouched. Existing declared previews are preserved. Images count toward the same 300-file / 20 MiB limits. Capture, resource, script, and size-limit failures fail the registered publication attempt and preserve the previous active set. Local preview without generated or declared images uses the Open screen fallback.
