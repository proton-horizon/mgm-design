...
The template installs the publisher's dependencies and Chromium, then adds `--previews` to generate small overview images before upload. Images show the designs without running every mock on the viewer's device; zooming and Interact load the working screens. Generation leaves source files untouched and counts toward the 300-file / 20 MiB bundle limits. For manual use, run `pnpm install --frozen-lockfile` and `pnpm exec playwright install --with-deps chromium` in the MGM checkout first. Update the destination before publishing the optional frame `preview` field; see the [capture contract](docs/backend.md#publisher-and-api).
